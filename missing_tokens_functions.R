...
get_counts_unique <- function(df, message_col, multinomial = FALSE, ngrams = FALSE, stop_words = NULL, stem_terms = NULL){
  quo_col <- enquo(message_col)

  stem_pattern = paste(stem_words[[1]], collapse = "|")
  ...
  if(multinomial){
    ...
    if(!is.null(stem_terms)){
      ...
      frame %>% mutate(token = ifelse(is.na(str_extract(token, stem_terms)),
                                      token,
                                      stem_terms %>% filter(stem_terms[[1]] == str_extract(token, stem_terms)) %>% pluck(2)))
...
get_prob_tables <- function(spam_train, ham_train, multinomial = FALSE, ngrams = FALSE, stop_words = NULL){
  #word counts for both groups
  word_counts_spam <- get_counts_unique(spam_train, message, multinomial, ngrams, stop_words)
  word_counts_ham <- get_counts_unique(ham_train, message, multinomial, ngrams, stop_words)
